...
| Standing OAuth access outside every login control | HIGH | A connected app holds broad standing API access — Full scope, a never-expiring refresh token, or both — while something means nobody would notice it being used: tokens matching no current app, tokens unused for months, or connected apps exempted from login IP enforcement. A refresh token is not a session, so MFA, IP ranges and SSO engage once at authorisation and never again, and the exchange produces no `LoginHistory` row. The path the 2025-26 campaigns took: tokens taken from the integration vendor, replayed against the org |
| Self-service registration into an over-shared portal | HIGH | A live Experience Cloud site accepts self-registration **and** the external sharing model or portal-reachable code gives any account holder more than their own records. Every other chain starting from an authenticated external user assumes the attacker already has an account; this one says anyone may have one, for the cost of an email address. Nothing is bypassed — MFA, IP ranges and SSO all behave as configured |
| Live session ID handed to an external endpoint | HIGH | A workflow outbound message includes the Salesforce session ID, so each firing sends a working credential to a third party — a supported option, which is why it survives. The org's exposure equals the weakest party that ever operated that endpoint. The remaining steps are why a replayed session would be neither blocked nor recorded: cleartext transit, unbound sessions, absent monitoring |
| Org files served to anonymous callers, with no record of what left | HIGH | Public Documents or static resources, content links with no expiry or password, or guest file access on a site — **and** no classification or monitoring to establish what the files held or who took them. Files are the blind spot in a sharing model: OWD, sharing rules and FLS govern records, none of them govern a static resource served from a URL. Claims no pivot into org data, because there is none |
| Visualforce XSS pattern reaching a privileged session | HIGH | Markup renders data unencoded (`escape="false"`, a merge field in a `<script>` block without JSENCODE, or one in an `href`/`src`/`action` attribute), the browser-side protections that would blunt an injected script are weakened, **and** privileged accounts exist to target. Does not claim the XSS is exploitable — markup cannot show whether an attacker controls the rendered value. A prioritisation of which pages to read first |
