...
export class IntegrationLeastPrivilegeCheck implements SecurityCheck {
  ...
  async run(ctx: AuditContext): Promise<CheckResult> {
    ...
    for (const row of psa) {
      ...
      const viaProfile = ps.IsOwnedByProfile === true;
      // A profile-owned permission set's Name is Salesforce's autogenerated internal string
      // (id-shaped, e.g. "X00e5w000001AbCd"), not the profile's label — naming it in a finding
      // would tell the reader nothing. The account's own profileName (from Profile.Name, already
      // carried on every resolved IntegrationAccount) is the human-readable name and needs no new
      // field. Mirrors the same fix applied to psMeta below for grantPathNote.
      const grantName = viaProfile ? (account.profileName ?? ps.Name) : ps.Name;
      for (const [permission, label] of Object.entries(ALL_PERMS)) {
        if (ps[permission] === true) {
          grants.push({ account, permission, label, viaProfile, grantName });
        }
      }
    }
    ...
    if (objPerms && objPerms.length > 0) {
      ...
      for (const p of psa) {
        if (p.PermissionSet) {
          const viaProfile = p.PermissionSet.IsOwnedByProfile === true;
          // A profile-owned permission set's Name is Salesforce's autogenerated internal string
          // (id-shaped, e.g. "X00e5w000001AbCd"), not the profile's label — naming it would tell
          // the reader nothing. The assignee's own profileName (from Profile.Name, already carried
          // on every resolved IntegrationAccount) is the human-readable name and needs no new field.
          const name = viaProfile ? (byId.get(p.AssigneeId)?.profileName ?? p.PermissionSet.Name) : p.PermissionSet.Name;
          psMeta.set(p.PermissionSetId, { name, viaProfile });
        }
      }
