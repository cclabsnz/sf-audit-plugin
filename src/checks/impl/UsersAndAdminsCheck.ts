...
interface PsaRecord {
  Assignee: { Username: string; Name?: string; Profile?: { Name: string } };
...
export class UsersAndAdminsCheck implements SecurityCheck {
  ...
  async run(ctx: AuditContext): Promise<CheckResult> {
    ...
    const modifyAllResult = await ctx.soql.query<PsaRecord>(
      'SELECT Assignee.Username, Assignee.Name, Assignee.Profile.Name, PermissionSet.Name, PermissionSet.IsOwnedByProfile FROM PermissionSetAssignment WHERE PermissionSet.PermissionsModifyAllData = true AND Assignee.IsActive = true'
    );
    ...
    findings.push({
      ...
      affectedItems: modifyAllUsers.map((r) => r.Assignee.Username),
    });

    // View All Data
    const viewAllResult = await ctx.soql.query<PsaRecord>(
      'SELECT Assignee.Username, Assignee.Name, PermissionSet.Name, PermissionSet.IsOwnedByProfile FROM PermissionSetAssignment WHERE PermissionSet.PermissionsViewAllData = true AND Assignee.IsActive = true'
    );
    ...
    findings.push({
      ...
      affectedItems: viewAllUsers.map((r) => r.Assignee.Username),
    });

    // Customize Application
    const customizeAppResult = await ctx.soql.query<PsaRecord>(
      'SELECT Assignee.Username, PermissionSet.Name, PermissionSet.IsOwnedByProfile FROM PermissionSetAssignment WHERE PermissionSet.PermissionsCustomizeApplication = true AND Assignee.IsActive = true'
    );
    ...
    if (customizeAppCount > 5) {
      findings.push({
        ...
        affectedItems: customizeAppUsers.map((r) => r.Assignee.Username),
      });
    }

    // Author Apex
    const authorApexResult = await ctx.soql.query<PsaRecord>(
      'SELECT Assignee.Username, PermissionSet.Name FROM PermissionSetAssignment WHERE PermissionSet.PermissionsAuthorApex = true AND Assignee.IsActive = true'
    );
    ...
    if (authorApexCount > 3) {
      findings.push({
        ...
        affectedItems: authorApexUsers.map((r) => r.Assignee.Username),
      });
    }
